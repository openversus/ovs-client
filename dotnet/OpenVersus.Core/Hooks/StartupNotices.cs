...
public static unsafe class StartupNotices
{
	...
	private static bool Attempt()
	{
		...
		if (age < SettleMs)
		{
			if (verbose) s_log?.Trace($"startup notices attempt {s_attempts}: instance 0x{GameUi.FighterGameInstance:X} is {age} ms old; waiting");
			return false;
		}
		var (frontend, stateWidget) = GameUi.FrontendState();
		if (frontend == 0 || stateWidget == 0)
		{
			if (verbose) s_log?.Trace($"startup notices attempt {s_attempts}: frontend 0x{frontend:X}, state widget 0x{stateWidget:X}; waiting");
			return false;
		}
		...
		if (!s_toastDone)
		{
			...
			nint widgetClass = 0;
			if (manager != 0) Memory.CodeWriter.TryRead(manager + Mvs.NotificationManagerDefaultWidgetClass, out widgetClass);
