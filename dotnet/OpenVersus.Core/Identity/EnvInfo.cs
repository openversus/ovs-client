...
public sealed class EnvInfo
{
	...
	private static string FindEpicId()
	{
		try
		{
			...
			if (string.IsNullOrEmpty(local)) return "Unknown";
	...
	private static unsafe string ReadBaseboardSerial()
	{
		try
		{
			...
			fixed (byte* p = buffer)
				if (Firmware.GetSystemFirmwareTable(Firmware.RSMB, 0, p, size) != size) return "Unknown";
			...
			while (pos + 4 <= end)
			{
				...
				if (length < 4 || pos + length > end || type == 127) break;
				if (type == 2 && length >= 8)
				{
				...
				int q = pos + length;
				while (q + 1 < end && !(buffer[q] == 0 && buffer[q + 1] == 0)) q++;
				pos = q + 2;
			}
		}
		catch { }
	...
	{
		int n = 0;
		while (at + n < end && buffer[at + n] != 0) n++;
		return n;
	}
}
