...
public sealed class AutoUpdate(string serverUrl, string pluginPath, IHttpTransport http, IHttpTransport download, Log log)
{
	...
	public void Run()
	{
		...
		if (url == null) { log.Warn("[AutoUpdate] Failed to parse server URL"); return; }

		var result = http.Get(url, TimeSpan.FromSeconds(5));
		if (!result.Ok) { log.Warn($"[AutoUpdate] Version check failed ({result.Error ?? $"HTTP {result.Status}"})"); return; }
		var info = Parse(result.Text);
		if (info == null) { log.Warn("[AutoUpdate] Version check returned something that is not JSON"); return; }
		log.Debug($"[AutoUpdate] Current: {OvsVersion.Current}, Latest: {info.LatestVersion}");
		if (info.IsLatest) { log.Info("[AutoUpdate] Already up to date"); return; }
		if (string.IsNullOrEmpty(info.LatestVersion) || string.IsNullOrEmpty(info.DownloadUrl)) { log.Warn("[AutoUpdate] Missing version/URL in response, skipping"); return; }
	...
	private void Install(VersionInfo info)
	{
		...
		if (url == null) { log.Warn($"[AutoUpdate] Bad download URL {info.DownloadUrl}"); return; }
		log.Info($"[AutoUpdate] Starting download from: {url}");
		var result = download.Get(url, TimeSpan.FromSeconds(60));
		if (!result.Ok) { log.Warn($"[AutoUpdate] Download failed ({result.Error ?? $"HTTP {result.Status}"})"); return; }
		log.Info($"[AutoUpdate] Downloaded {result.Body.Length} bytes");
		if (result.Body.Length < 10000) { log.Warn($"[AutoUpdate] Download update file too small ({result.Body.Length} bytes), aborting"); return; }
