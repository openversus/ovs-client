...
public sealed unsafe class WinHttpTransport(string agent = "OVS/1.0", bool useSystemProxy = false) : IHttpTransport
{
	...
	private HttpResult Send(Uri url, string verb, string? headers, ReadOnlySpan<byte> body, TimeSpan timeout)
	{
		...
		try
		{
			...
			if (session == 0) return HttpResult.Failed($"WinHttpOpen failed ({Marshal.GetLastPInvokeError()})");
			...
			if (connect == 0) return HttpResult.Failed($"WinHttpConnect failed ({Marshal.GetLastPInvokeError()})");
			...
			if (request == 0) return HttpResult.Failed($"WinHttpOpenRequest failed ({Marshal.GetLastPInvokeError()})");
			...
			if (!sent) return HttpResult.Failed($"WinHttpSendRequest failed ({Marshal.GetLastPInvokeError()})");
			if (!WinHttp.WinHttpReceiveResponse(request, 0)) return HttpResult.Failed($"WinHttpReceiveResponse failed ({Marshal.GetLastPInvokeError()})");
		...
		{
			if (request != 0) WinHttp.WinHttpCloseHandle(request);
			if (connect != 0) WinHttp.WinHttpCloseHandle(connect);
			if (session != 0) WinHttp.WinHttpCloseHandle(session);
		}
	}
}
