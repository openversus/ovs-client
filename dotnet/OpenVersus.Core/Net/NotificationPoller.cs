...
public sealed class NotificationPoller(string serverUrl, IHttpTransport http, ObjectFinder finder, GameImage image, Log log)
{
	...
	public void Start()
	{
		if (_running) { log.Info("[NotifPoller] Already running"); return; }
		if (Urls.Join(serverUrl, "/ovs/notifications") is not { } url) { log.Warn($"[NotifPoller] Failed to parse server URL: {serverUrl}"); return; }
		_running = true;
	...
	public static List<Notification> Parse(string json)
	{
		...
		try
		{
			...
			{
				if (e.ValueKind != JsonValueKind.Object || !e.TryGetProperty("type", out var type) || type.ValueKind != JsonValueKind.String) continue;
	...
	private void CancelMatchOnGameThread()
	{
		...
		if (preMatch == 0) { log.Info("[CancelMatch] No AMvsPreMatchGameState — player is in active gameplay, ignoring"); return; }

		if (!Memory.CodeWriter.TryRead(preMatch + 0x320, out nint stateMachine) || !Plausible(stateMachine)) { log.Info($"[CancelMatch] StateMachine ptr invalid (0x{stateMachine:X})"); return; }
		if (!Memory.CodeWriter.TryRead(stateMachine + 0x58, out nint currentState) || !Plausible(currentState)) { log.Info($"[CancelMatch] CurrentState ptr invalid (0x{currentState:X}) — no-op"); return; }
		...
		if (handler == null) { log.Info("[CancelMatch] UFunction HandleTransitionToGameplayFailed on MvsPreMatchTransitioningToGameplayState not found — no-op"); return; }
